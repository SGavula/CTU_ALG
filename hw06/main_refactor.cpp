...
void updateNumOfDeletedNodes(Node *node) {
  ...
  while(node != nullptr) {
    ...
    if(node->key == 386) {
      ...
      // cout << "Update node: " << node->key << " " << node->maxDeletedChilds << " Left: " << node->left->key << endl;
    }
    node = node->parent;
  }
};
...
Node *rightRotate(Node *y, Utils *utils) {
  ...
  updateNumOfDeletedNodes1(y);
  updateNumOfDeletedNodes1(x);
...
void doConsolidation(Utils *utils) {
  ...
  if(deletedNode != nullptr) {
    ...
    if(utils->root != nullptr) {
      updateNumOfDeletedNodes1(utils->root);
    }
  }else {
    utils->stop = true;
