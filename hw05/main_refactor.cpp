...
bool checkCounter(Node* root);
bool checkCounterUtil(Node* node, int threshold);
...
int getSize(Node* root) {
    ...
    // Recursive calculation: size = size of left subtree + size of right subtree + 1 (for the current node)
    return getSize(root->left) + getSize(root->right) + 1;
}

bool checkCounter(Node* root, int value) {
    return checkCounterUtil(root, value);
}
...
bool checkCounterUtil(Node* node, int threshold) {
    ...
    // Check the current node's counter value
    if (node->branch >= threshold) {
        return false;  // The condition is not satisfied
    }

    // Recursively check the left and right subtrees
    return checkCounterUtil(node->left, threshold) && checkCounterUtil(node->right, threshold);
}
...
void processOperations(int k) {
    ...
    while (cin >> operation >> key) {
        ...
        if (operation == 'I') {
            ...
            // auto it = std::find(testVector.begin(), testVector.end(), key);
        ...
        // if(isBST(utils->root) == false) {
        //     cout << operation << " " << key << endl;
        //     cout << "TREE IS NOT BST" << endl;
        //     printTree(utils->root, 0);
        //     break;
        // };

        // if(checkCounter(utils->root, utils->branchLength) == false) {
        //     cout << operation << " " << key << endl;
        //     cout << "THERE IS BRANCH" << endl;
        //     printTree(utils->root, 0);
        //     break;
        // }
        // if(testVector.size() != getSize(utils->root)) {
        //     cout << "Size: " << testVector.size() << endl;
        //     cout << "Size of tree: " << getSize(utils->root) << endl;
        //     cout << operation << " " << key << endl;
